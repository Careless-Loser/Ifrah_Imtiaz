...
let backgroundAudio = new Audio('/Ifrah_Imtiazgithub.io/Index Ref/img/Clipchamp.mp3'); // Create an audio object

toggleSoundButton.addEventListener('click', () => {
    isSoundOn = !isSoundOn;
    toggleSoundButton.textContent = isSoundOn ? 'Sound: On' : 'Sound: Off';
    if (isSoundOn) {
        backgroundAudio.play(); // Start playing audio when sound is toggled on
    } else {
        backgroundAudio.pause(); // Pause audio when sound is toggled off
    }
});
